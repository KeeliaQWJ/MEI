...
class SmilesDataset(dgl.data.DGLDataset):
    ...
    def save(self):
        ...
        try:
            ...
            print(f'Error saving reactant graphs: {e}')
        
        try:
            dgl.save_graphs(self.path + '_product_graphs.bin', self.product_graphs)
        except Exception as e:
            print(f'Error saving product graphs: {e}')
...
def read_data(dataset, mode):
    path = os.path.join('../data', str(dataset), str(mode) + '.csv')
    ...
    with open(path) as f:
        for line in f.readlines():
            ...
            product_smiles = product_smiles.replace('([H])', '').replace('[H]', '').replace('([H+])', '').replace('[H+].', '').replace('[H+].', '')
            reactant_smiles = reactant_smiles.replace('([H])', '').replace('[H]', '').replace('([H+])', '').replace('[H+].', '').replace('[H+].', '')
            product_smiles = product_smiles.replace('[1*]', '').replace('[2*]', '').replace('[3*]', '').replace('[4*]', '').replace('()', '')
            product_smiles = product_smiles.replace('(*)', '').replace('*.', '').replace('*', '').replace('..', '')
            reactant_smiles = reactant_smiles.replace('[1*]', '').replace('[2*]', '').replace('[3*]', '').replace('[4*]', '').replace('()', '')
            reactant_smiles = reactant_smiles.replace('(*)', '').replace('*.', '').replace('*', '').replace('..', '')
            # skip the first line
            if len(idx) == 0:
                continue
            ...
            try:
                ...
                print(f"Error processing product SMILES: {e}")
                product_smiles = ''  
                reactant_smiles = ''
                continue
...
def load_data(args):
    ...
    if os.path.exists('../data/' + args.dataset + '/cache/'):
        ...
        train_dataset = SmilesDataset(args, 'train', feature_encoder, raw_graphs=train_graphs)
        valid_dataset = SmilesDataset(args, 'valid', feature_encoder, raw_graphs=valid_graphs)
        test_dataset = SmilesDataset(args, 'test', feature_encoder, raw_graphs=test_graphs)
