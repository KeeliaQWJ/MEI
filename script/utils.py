...
class BestMeter(object):
    ...
    def update(self, value):
        """
        Update the best value if the incoming value is better.
        Returns True if the best value is updated.
        """
        updated = False
        if self.best_type == 'min' and value < self.best:
            self.best = value
            self.count = 0
            updated = True
        elif self.best_type != 'min' and value > self.best:
            self.best = value
            self.count = 0
            updated = True
        return updated
